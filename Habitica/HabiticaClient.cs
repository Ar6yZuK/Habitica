...
public class HabiticaClient : IDisposable
{
	...
	private async Task<TData?> RequestAsync<TData>(string additionForResource, HttpMethod method, CancellationToken cancellationToken)
	{
		...
		TData? data = await EnsureSuccess<TData>(responseMessage, cancellationToken);
		return data;
	}
	...
	private static async Task<TData?> EnsureSuccess<TData>(HttpResponseMessage responseMessage, CancellationToken cancellationToken) 
	{
